...
TIKTOK_SIGN_API_KEY=        # Euler Stream key for TikTok (see below)
PORT=3000
MAX_SESSIONS=50             # max concurrent chat sessions
...
- **`server.js` (Node server):** this is what connects to Twitch, Kick, TikTok,
  ...
  immediately: it gets a grace period (6 hours) during which it stays alive
  and keeps capturing messages (and accumulating the header stats —
  subscriptions, gifts, bits — for the whole stream, not just while a browser
  is open); if any client returns the period is cancelled, otherwise it is
  destroyed along with its platform connections. The aggregate stats can also
  be cleared at any time with the **Reset** button in the stats panel.
...
with its own clients and feed, and stays alive for 6 hours after the last
...
- **Twitch:** the server connects to the public chat server over WebSocket
  ...
  viewers. The **live viewer count** is fetched from the Twitch Helix API
  (`GET /helix/streams`) using an app access token derived from
  `TWITCH_CLIENT_ID`/`TWITCH_CLIENT_SECRET` (the same credentials used for
  OAuth). It's polled every 60 s and shown in the TW status pill; if those
  credentials are not configured, the count is simply not shown.
- **Kick:** there is no official read API without login, so the server connects
  ...
  may break if Kick changes something on their end. The **live viewer count**
  comes from the same v2 channel endpoint used for chatroom lookup
  (`livestream.viewer_count`), polled every 60 s and shown in the KK status pill.
...
TikTok is the only platform that needs an extra value in `.env`. It requires an
**Euler Stream** API key to sign each connection (it will not work without one):
...
2. Copy the API key from the dashboard into `.env`:
   ...
   ```env
   TIKTOK_SIGN_API_KEY=your_key_here
   ```
