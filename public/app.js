...
// بعد عودة OAuth من تويتش: السيرفر خزّن الـ token في كوكي مؤقت (لمرة واحدة).
// نسأل هذا المسار عند الإقلاع، فإن وُجد token نلتقطه ونحفظه محلياً.
// هذا أكثر موثوقية من قراءة الـ URL fragment الذي قد لا يصل عبر بروكسي/HTTPS.
...
// يتحقق إن كان هناك خطأ OAuth مؤجَّل (من كوكي) ليُعرض للمستخدم.
...
// =====================================================================
(async function boot() {
  // التقط نتيجة OAuth إن كنا عائدين من تويتش (السيرفر وضعها في كوكي مؤقت)
  ...
  // استئناف تلقائي إن كانت هناك قناة محفوظة
  ...
  // أبلغ المستخدم بنتيجة الربط بعد أن تكون الواجهة جاهزة (في الشات)
  if (justConnected) {
    addSystemMessage(twitchLogin ? `تم ربط حساب تويتش: @${twitchLogin}` : "تم ربط حساب تويتش.");
  } else if (oauthError) {
    const reasons = {
      nocode: "لم يصل رمز التفويض من تويتش",
      notconfigured: "OAuth غير مُهيّأ على السيرفر (TWITCH_CLIENT_ID/SECRET)",
      network: "تعذّر الاتصال بخادم تويتش",
      token: "رفض تويتش تبادل الرمز بـ token (تحقّق من TWITCH_REDIRECT_URI)",
      noaccess: "لم يُرجع تويتش access_token",
    };
    addSystemMessage("فشل ربط حساب تويتش: " + (reasons[oauthError] || oauthError));
  }
})();
