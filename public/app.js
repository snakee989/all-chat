...
const statFollowsBtn = document.getElementById("stat-follows-btn");
const statSubsCount = document.getElementById("stat-subs-count");
const statBitsCount = document.getElementById("stat-bits-count");
const statFollowsCount = document.getElementById("stat-follows-count");
...
// Aggregate stats received from the server (subs / gifts / bits / follows).
...
let stats = { subs: [], gifts: [], bits: [], follows: [] };
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (msg.type === "snapshot") {
      ...
      for (const m of msg.backlog) {
        if (m.id > lastSeenId) {
          if (m.type === "event") {
            addEventMessage(m.platform, m.kind, m.who, m.text, m.message, false);
          } else {
            addChatMessage(m.platform, m.username, m.content, m.color, m.emotes, false, m.bits);
          }
          lastSeenId = m.id;
        }
      }
    ...
    if (msg.type === "chat") {
      ...
      addChatMessage(msg.platform, msg.username, msg.content, msg.color, msg.emotes, true, msg.bits);
      return;
    }
    ...
    if (msg.type === "event") {
      if (typeof msg.id === "number" && msg.id <= lastSeenId) return;
      if (typeof msg.id === "number") lastSeenId = msg.id;
      addEventMessage(msg.platform, msg.kind, msg.who, msg.text, msg.message, true);
      return;
    }
...
function addEventMessage(platform, kind, who, text, message, autoscroll) {
...
// =====================================================================
// Stats panels (subs / bits / follows)
// =====================================================================
...
function applyStats(next) {
  ...
  stats = {
    ...
    follows: Array.isArray(next.follows) ? next.follows : [],
  };
  refreshStatsUI();
...
// subscribers + gifters (both are "subscription" activity); bits and follows
// show their own totals.
function refreshStatsUI() {
  ...
  statFollowsCount.textContent = String(stats.follows.length);
}
...
function openStatsPanel(kind) {
  ...
  [statSubsBtn, statBitsBtn, statFollowsBtn].forEach((b) =>
...
function closeStatsPanel() {
  statsPanelOpen = null;
  statPanel.hidden = true;
  [statSubsBtn, statBitsBtn, statFollowsBtn].forEach((b) => b.classList.remove("is-active"));
}

function kindButton(kind) {
  return kind === "subs" ? statSubsBtn : kind === "bits" ? statBitsBtn : statFollowsBtn;
}
...
function renderStatsPanel(kind) {
  ...
  // follows
  statPanelTitle.textContent = "Follows";
  statPanelBody.innerHTML = "";
  statPanelBody.appendChild(buildStatsSection("Followers", stats.follows, () => ""));
}
...
statFollowsBtn.addEventListener("click", () => openStatsPanel("follows"));
...
function addChatMessage(platform, username, content, color, emotes, autoscroll, bits) {
  const wasAtBottom = autoscroll
    ? feed.scrollTop + feed.clientHeight >= feed.scrollHeight - 40
    : false;
...
connectTwitchBtn.addEventListener("click", () => {
  ...
  // Start OAuth: send the user to /auth/twitch. A simple random state guards CSRF.
  const state = Math.random().toString(36).slice(2);
  sessionStorage.setItem("twitch_oauth_state", state);
  // We pass state as a query param; the server redirects Twitch to the callback.
