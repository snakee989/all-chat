...
function readStoredJoin() {
  try {
    ...
    if (!tw && !kk) return null;
    return { twitch: tw, kick: kk };
  } catch {
    return null;
  }
}

function writeStoredJoin(twitchUser, kickUser) {
  try {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ twitch: twitchUser || "", kick: kickUser || "" })
    );
  } catch {
...
gateForm.addEventListener("submit", (e) => {
  ...
  writeStoredJoin(twitchUser, kickUser);
...
function connectServer() {
  ...
  socket.addEventListener("open", () => {
    ...
    socket.send(
      JSON.stringify({ type: "join", twitch: joinedTwitch, kick: joinedKick, lastId: lastSeenId })
    );
  });
...
(function boot() {
  ...
  if (saved) {
    ...
    openStage(saved.twitch, saved.kick);
  }
})();
