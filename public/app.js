...
function addEventMessage(platform, kind, who, text, message, autoscroll, sharedFrom, sharedAvatar, badges) {
  ...
  // Twitch badges (sub/founder/etc.) sit right after the label, so e.g. a
  // founder's resub shows their founder badge inline with the event text.
  if (platform === "tw") {
    const badgeFrag = buildTwitchBadges(badges);
    if (badgeFrag.childNodes.length) body.appendChild(badgeFrag);
  }
...
function addChatMessage(platform, username, content, color, emotes, autoscroll, bits, sharedFrom, sharedAvatar, badges) {
  ...
  // Twitch badges (sub/mod/vip/...) render right after the username, matching
  // twitch.tv. Only Twitch rows carry the `badges` field.
  if (platform === "tw") {
    const badgeFrag = buildTwitchBadges(badges);
