...
// ---------- Alert sound overlays (StreamElements / Streamlabs / etc.) ----------
// Each saved URL becomes a hidden iframe on the chat screen that plays its
// alert sound on events. URLs live only in localStorage and are NEVER sent
// to the server (they are personal overlay tokens). Mirrors unified-chat-lite.
const ALERTS_STORAGE_KEY = "dualchat:alerts";
const alertsRowsEl = document.getElementById("alerts-rows");
...
// A valid alert URL is any absolute https: URL. We deliberately don't restrict
// the host — StreamElements, Streamlabs and others all use different domains
// and per-account subdomains.
...
function writeAlertUrls(urls) {
  try {
    localStorage.setItem(ALERTS_STORAGE_KEY, JSON.stringify(urls));
  } catch (_) {}
}

// Adds one editor row (input + remove button). Called on load for each saved
// URL, and when the user clicks "+ Add alert URL".
function addAlertRow(value = "") {
  const row = document.createElement("div");
  row.className = "alerts-row";
  const input = document.createElement("input");
  input.type = "url";
  input.className = "alerts-input";
  input.placeholder = "https://streamelements.com/overlay/…";
  input.spellcheck = false;
  input.value = value;
  const remove = document.createElement("button");
  remove.type = "button";
  remove.className = "alerts-remove-btn";
  remove.textContent = "×";
  remove.title = "Remove";
  remove.addEventListener("click", () => {
    row.remove();
    saveAlertsFromEditor();
    refreshAlertCount();
  });
  row.appendChild(input);
  row.appendChild(remove);
  alertsRowsEl.appendChild(row);
  return input;
}

// Re-reads every row, keeps only valid https: URLs (deduped, order preserved),
// persists them, and updates the saved badge. Invalid rows are flagged so the
// user sees what to fix. Called on every change (input/remove/add) so the
// editor and storage never drift apart — simpler than a separate Apply button.
function saveAlertsFromEditor() {
  const seen = new Set();
  const urls = [];
  for (const input of alertsRowsEl.querySelectorAll(".alerts-input")) {
...
// Updates the small count badge next to the editor label.
function refreshAlertCount() {
  const count = readAlertUrls().length;
  if (count > 0) {
...
// Render (or reconcile) the hidden overlay iframes on the chat screen. Called
// when the stage opens and whenever the alert list changes while chat is open.
let alertUrls = [];

// Third-party Twitch emotes (FFZ/BTTV/7TV). The server sends a per-channel +
// global name→url map as {type:"emotes"} (and in the snapshot). We hold it
// here and match whitespace-separated words against it during rendering.
...
function renderAlertFrames() {
  ...
  const existing = new Set([...alertFramesEl.children].map((f) => f.src));
...
alertsAddBtn.addEventListener("click", () => {
  const input = addAlertRow("");
  input.focus();
});

// Save on every keystroke/paste so a refresh never loses an unsaved URL.
alertsRowsEl.addEventListener("input", () => {
  saveAlertsFromEditor();
  refreshAlertCount();
  // If chat is already open, keep the live iframes in sync.
  if (!stage.hidden) {
    alertUrls = readAlertUrls();
    renderAlertFrames();
  }
});

// One-time sound unlock. Browsers block autoplay until the user has interacted
// with the page; the first pointerdown anywhere counts, so we arm a one-shot
// listener that pokes each overlay iframe to unmute its audio.
...
// Restore any saved alert URLs into the editor rows at boot.
(function initAlertEditor() {
  const urls = readAlertUrls();
  if (urls.length) {
    urls.forEach(addAlertRow);
  }
  refreshAlertCount();
})();

// ---------- Settings panel (live channel management) ----------
// Opened from the ⚙ button in the header. Lists each active channel with
// Edit/Remove actions, plus the alert-overlay editor (moved here from the
// gate). Editing/removing a channel applies the new channel set by re-joining
// (a fresh WS `join` with the updated channels + lastId=0 so the server
// treats it as a brand-new session and the feed is wiped).
const settingsBtn = document.getElementById("settings-btn");
const settingsPanel = document.getElementById("settings-panel");
const settingsCloseBtn = document.getElementById("settings-close");
const settingsChannelsEl = document.getElementById("settings-channels");

// Platform metadata for the rows: label, accent class, and which `joined*`
// variable holds the value. Kick is special — it carries up to 3 comma-joined
// channels, so its row lets you edit/remove each individually.
const PLATFORM_ROWS = [
  { code: "tw", label: "Twitch", accent: "tw" },
  { code: "kk", label: "Kick", accent: "kk" },
  { code: "tt", label: "TikTok", accent: "tt" },
  { code: "yt", label: "YouTube", accent: "yt" },
];

function openSettingsPanel() {
  // Only one panel-style overlay open at a time (mirrors the stat panel).
...
// Returns the current channel set as a plain object the rejoin helper uses.
...
// Renders the channel rows from the live `joined*` values. Re-runs every time
// the panel opens or a row is removed/edited, so it always reflects the truth.
function renderSettingsChannels() {
  settingsChannelsEl.innerHTML = "";
  const ch = currentChannels();
  // Twitch / TikTok / YouTube — one input each.
  for (const p of PLATFORM_ROWS) {
    if (p.code === "kk") continue;
    const value = ch[p.code];
    if (!value) continue;
    settingsChannelsEl.appendChild(buildSingleChannelRow(p, value));
  }
  // Kick — one row per channel (up to 3), each independently editable/removable.
  const kickChannels = joinedKick ? String(joinedKick).split(",").map((s) => s.trim()).filter(Boolean) : [];
  kickChannels.forEach((name, idx) => {
    settingsChannelsEl.appendChild(buildKickChannelRow(name, idx));
  });
}

// A row for a single-value platform (Twitch / TikTok / YouTube).
function buildSingleChannelRow(p, currentValue) {
  const row = document.createElement("div");
  row.className = "settings-channel-row";

  const badge = document.createElement("span");
  badge.className = `settings-channel-badge ${p.accent}`;
  badge.textContent = p.code.toUpperCase();

  const name = document.createElement("span");
  name.className = "settings-channel-name";
  name.textContent = currentValue;

  const actions = document.createElement("div");
  actions.className = "settings-channel-actions";

  const editBtn = document.createElement("button");
  editBtn.type = "button";
  editBtn.className = "settings-channel-btn edit";
  editBtn.textContent = "Edit";
  editBtn.addEventListener("click", () => {
    startChannelEdit(row, name, editBtn, (newVal) => {
      if (newVal === null) return; // cancelled
      const next = currentChannels();
      next[p.code] = newVal;
      applyChannels(next.tw, next.kk, next.tt, next.yt);
    });
  });

  const removeBtn = document.createElement("button");
  removeBtn.type = "button";
  removeBtn.className = "settings-channel-btn remove";
  removeBtn.textContent = "Remove";
  removeBtn.addEventListener("click", () => {
    if (!confirmKeepChannels()) return;
    const next = currentChannels();
    next[p.code] = null;
    applyChannels(next.tw, next.kk, next.tt, next.yt);
  });
...
// A row for ONE Kick channel (by index in the comma-joined list). Edit/Remove
// operate on that specific channel only — the others stay.
function buildKickChannelRow(currentName, index) {
  const row = document.createElement("div");
  row.className = "settings-channel-row";

  const badge = document.createElement("span");
  badge.className = "settings-channel-badge kk";
  badge.textContent = "KK";

  const name = document.createElement("span");
  name.className = "settings-channel-name";
  name.textContent = currentName + (index === 0 ? "  (primary)" : "");
  ...
  editBtn.addEventListener("click", () => {
    startChannelEdit(row, name, editBtn, (newVal) => {
      ...
      const list = joinedKick ? String(joinedKick).split(",").map((s) => s.trim()) : [];
      list[index] = newVal;
      const next = currentChannels();
      next.kk = list.filter(Boolean).join(",");
      applyChannels(next.tw, next.kk, next.tt, next.yt);
    });
  });
  ...
  removeBtn.addEventListener("click", () => {
    ...
    list.splice(index, 1);
    const next = currentChannels();
    next.kk = list.filter(Boolean).join(",") || null;
    applyChannels(next.tw, next.kk, next.tt, next.yt);
  });
...
// Turns a row's name span into an inline input with Save/Cancel. onSave
// receives the new value (already normalized+trimmed), or null if cancelled.
function startChannelEdit(row, nameEl, editBtn, onSave) {
  const original = String(nameEl.textContent || "").replace(/\s*\(primary\)\s*$/, "").trim();
  const input = document.createElement("input");
  input.type = "text";
  input.className = "settings-channel-edit";
  ...
  saveBtn.type = "button";
  saveBtn.className = "settings-channel-btn save";
  saveBtn.textContent = "Save";

  const cancelBtn = document.createElement("button");
  cancelBtn.type = "button";
  cancelBtn.className = "settings-channel-btn cancel";
  cancelBtn.textContent = "Cancel";

  // Hide the name + edit button while editing; show input + save/cancel.
  nameEl.hidden = true;
  editBtn.hidden = true;
  row.insertBefore(input, nameEl);
  const actionWrap = row.querySelector(".settings-channel-actions");
  // Disable the remove button while editing to avoid races.
  const removeBtn = actionWrap?.querySelector(".remove");
  if (removeBtn) removeBtn.disabled = true;
  actionWrap.insertBefore(saveBtn, editBtn);
  actionWrap.insertBefore(cancelBtn, editBtn);
  input.focus();
  input.select();
  ...
  const commit = (accept) => {
    ...
    if (accept) {
      ...
      // Normalize: only accept if changed and non-empty.
      if (v && v !== original) newVal = v;
    }
    // Restore the row; renderSettingsChannels() will rebuild it after apply.
    input.remove();
    saveBtn.remove();
    cancelBtn.remove();
    nameEl.hidden = false;
...
// Don't allow removing the LAST channel — that would be a no-channel join,
// which the server rejects. Direct the user to "Change" instead.
function confirmKeepChannels() {
  const ch = currentChannels();
  const kickCount = ch.kk ? String(ch.kk).split(",").filter(Boolean).length : 0;
  const total = (ch.tw ? 1 : 0) + kickCount + (ch.tt ? 1 : 0) + (ch.yt ? 1 : 0);
  if (total <= 1) {
    addSystemMessage("Can't remove the last channel — use “Change” to pick a different setup.");
    return false;
  }
  return true;
}

// Central re-join helper. Updates the joined* vars, persists them, wipes the
// feed cursor, and reconnects so the server treats it as a fresh join (new
// session key) — the old session enters its grace period and the feed starts
// empty with only the kept channels. Mirrors the gate submit + openStage path.
function applyChannels(tw, kk, tt, yt) {
  joinedTwitch = tw || null;
  joinedKick = kk || null;
  joinedTikTok = tt || null;
  joinedYouTube = yt || null;
  writeStoredJoin(joinedTwitch, joinedKick, joinedTikTok, joinedYouTube);

  // Drop the third-party emotes from the previous channel so stale KEKW/…
  // matches don't bleed into the new one. The new channel's map arrives in
  // the next snapshot / "emotes" broadcast.
  thirdPartyEmotes = new Map();

  // Reset cursor so the next snapshot is treated as a fresh connect (the
  // server's snapshot handler wipes the feed when lastSeenId === 0).
  lastSeenId = 0;
  feed.innerHTML = "";

  // Reset per-platform status text for any removed platform so its pill
  // doesn't keep showing "live" while the new socket connects.
  if (!joinedTwitch) twStatus.textContent = "not set";
  if (!joinedKick) kkStatus.textContent = "not set";
  if (!joinedTikTok) ttStatus.textContent = "not set";
  if (!joinedYouTube) ytStatus.textContent = "not set";

  // Stat icons only make sense with Twitch/Kick.
  statIcons.hidden = !(joinedTwitch || joinedKick);
  statPanel.hidden = true;
  statsPanelOpen = null;

  // Composer + video only for Twitch.
  if (!joinedTwitch) {
  ...
  // Reset the Hype Train bar (only Twitch had one running).
  resetHypeTrainBar();

  // Reconnect: closing the socket triggers the standard close handler, but
  // we don't want the exponential backoff path here — connect immediately.
  // The close handler uses event.target identity to detect a replaced socket
  // and skips its own scheduleReconnect() when we've moved on to a new one.
  if (reconnectTimer) {
  ...
  // Re-render the panel so the row we just edited/removed is up to date.
  renderSettingsChannels();
}
...
let twitchLogin = "";
// Proactive token refresh: a timer fires ~1 min before the access token
// expires and silently renews it via /api/twitch-refresh (which holds the
// refresh_token in an HttpOnly cookie). This keeps chat working for hours
// without the viewer having to re-authorize.
let twitchRefreshTimer = null;
// Token expiry held in memory (mirrors localStorage so the scheduler can chain
// a follow-up refresh after each successful renewal). 0 = unknown TTL.
let tokenExpiresAt = 0;
// In-flight refresh promise so two tabs/buttons can't refresh concurrently.
let twitchRefreshPromise = null;
// When a send fails due to an expired token, we refresh once and retry this
// content (avoids losing the message the viewer just typed).
let pendingSendRetryContent = null;
...
// ---------- Twitch badges ----------
// Twitch ships per-user badges in the IRC `badges` tag as "slug/version"
// pairs (e.g. "subscriber/12", "moderator/1"). Each global badge slug maps
// to a stable CDN UUID; the version is irrelevant for the global set. We
// render these as small inline images so a mod/sub/vip is visible at a
// glance — mirroring how twitch.tv and unified-chat-lite display them.
...
// Returns a DocumentFragment of <img> badge elements for a Twitch chat/event
// row. Empty for non-Twitch rows or rows with no badges. Safe by construction
// (DOM-built, no innerHTML), matching the rest of this file's rendering style.
function buildTwitchBadges(badges) {
  const frag = document.createDocumentFragment();
  if (!Array.isArray(badges)) return frag;
  for (const raw of badges) {
    const slug = String(raw).split("/")[0].toLowerCase();
    const id = TWITCH_BADGE_IDS[slug];
    if (!id) continue; // unknown/channel-custom badge — skip rather than show a broken img
    const img = document.createElement("img");
    img.className = "twitch-badge";
    img.src = `https://static-cdn.jtvnw.net/badges/v1/${id}/1`;
    img.alt = slug;
    img.title = slug;
    img.loading = "lazy";
...
function readStoredTwitch() {
  try {
    ...
    return {
      ...
      // ms epoch when the access token expires (0/absent = unknown → treat as
      // always-valid; the server-side refresh is the real safety net).
      expires_at: Number(p.expires_at) > 0 ? Number(p.expires_at) : 0,
    };
  } catch {
    return null;
  }
...
function writeStoredTwitch(accessToken, login, expiresAt) {
  try {
    if (accessToken) {
      localStorage.setItem(
        STORAGE_TWITCH,
...
function openStage(twitchUser, kickUser, tiktokUser, youtubeUser) {
  ...
  // Mount the alert-overlay iframes (StreamElements / Streamlabs / etc.) for
  // the saved URLs. Iframes load off-screen; only their audio plays. The audio
  // unlock is armed on the first user gesture (already done at boot).
  alertUrls = readAlertUrls();
  renderAlertFrames();
...
resetBtn.addEventListener("click", () => {
  ...
  statPanel.hidden = true;
  statsPanelOpen = null;
  // Close the Settings panel if it was open.
  settingsPanel.hidden = true;
  // Tear down the alert-overlay iframes so they stop playing while the user is
  // back on the channel-select screen. They're re-mounted on next openStage().
  alertUrls = [];
  if (alertFramesEl) alertFramesEl.innerHTML = "";
  // Hide the Hype Train bar so it doesn't linger across sessions.
  resetHypeTrainBar();
  // Drop third-party emotes from the previous channel.
  thirdPartyEmotes = new Map();
  clearStoredJoin();
  clearGateNotice();
  gate.hidden = false;
...
function connectServer() {
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (msg.type === "snapshot") {
      ...
      for (const m of msg.backlog) {
        if (m.id > lastSeenId) {
          if (m.type === "event") {
            addEventMessage(m.platform, m.kind, m.who, m.text, m.message, false, m.sharedFrom, m.sharedAvatar, m.badges);
          } else {
            addChatMessage(m.platform, m.username, m.content, m.color, m.emotes, false, m.bits, m.sharedFrom, m.sharedAvatar, m.badges);
          }
          lastSeenId = m.id;
        }
      }
      ...
      // Re-render any Hype Train in progress so a reconnecting viewer sees it
      // without waiting for the next poll tick.
      if (msg.hypeTrain) handleHypeTrain(msg.hypeTrain);
      else resetHypeTrainBar();

      // Adopt the third-party emotes map so a reconnecting viewer can render
      // KEKW/monkaS/… from the next message without waiting for the server to
      // re-broadcast. Cleared if the snapshot has no emotes yet (Twitch still
      // loading them or not configured).
      thirdPartyEmotes = msg.emotes ? new Map(Object.entries(msg.emotes)) : new Map();
    ...
    if (msg.type === "chat") {
      ...
      addChatMessage(msg.platform, msg.username, msg.content, msg.color, msg.emotes, true, msg.bits, msg.sharedFrom, msg.sharedAvatar, msg.badges);
      return;
    }
    ...
    if (msg.type === "event") {
      if (typeof msg.id === "number" && msg.id <= lastSeenId) return;
      if (typeof msg.id === "number") lastSeenId = msg.id;
      addEventMessage(msg.platform, msg.kind, msg.who, msg.text, msg.message, true, msg.sharedFrom, msg.sharedAvatar, msg.badges);
      return;
    }

    if (msg.type === "hype_train") {
      handleHypeTrain(msg);
      return;
    }

    if (msg.type === "emotes") {
      // Server pushed a refreshed third-party emotes map (FFZ/BTTV/7TV) for
      // this Twitch channel. Stored in a Map and read live by renderContentInto.
      thirdPartyEmotes = new Map(Object.entries(msg.emotes || {}));
      return;
    }
  ...
  socket.addEventListener("close", (event) => {
    // If this is the close of a socket we've already replaced (e.g. the
    // Settings panel triggered a re-join), the global `socket` now points at a
    // different/newer WebSocket — bail out and let the new connection own its
    // own lifecycle. Without this, every Settings edit would stack a second
    // reconnect attempt on top of the one we drive ourselves.
    if (socket && event.target && event.target !== socket) return;
...
// ---------- Hype Train (Twitch) ----------
// Polled server-side via anonymous GQL. Server sends {type:"hype_train",
// phase:"progress"|"end", level, progress, goal, hide_after_ms?}. We render a
// bar above the feed; on "end" we let the bar sit at its final % for a few
// seconds then hide it. Mirrors unified-chat-lite's rendering.
const hypeTrainBar = document.getElementById("hype-train-bar");
const hypeTrainLevelEl = document.getElementById("ht-level");
const hypeTrainProgressTextEl = document.getElementById("ht-progress-text");
const hypeTrainFillEl = document.getElementById("ht-fill");
...
function resetHypeTrainBar() {
  ...
  if (!hypeTrainBar) return;
  hypeTrainBar.hidden = true;
  hypeTrainBar.setAttribute("aria-hidden", "true");
}

function renderHypeTrain(data) {
  if (!hypeTrainBar) return;
  hypeTrainBar.hidden = false;
  hypeTrainBar.setAttribute("aria-hidden", "false");
  hypeTrainBar.dataset.phase = data.phase || "progress";
  if (hypeTrainLevelEl) hypeTrainLevelEl.textContent = data.level || 1;
  const goal = data.goal > 0 ? data.goal : 1;
  const pct = Math.min(Math.round(((data.progress || 0) / goal) * 100), 100);
  if (hypeTrainProgressTextEl) {
    hypeTrainProgressTextEl.textContent =
      data.phase === "end" ? `Ended (${pct}%)` : `${pct}%`;
  }
  if (hypeTrainFillEl) hypeTrainFillEl.style.width = `${pct}%`;
}
...
function handleHypeTrain(data) {
  ...
  // Cancel any pending hide so a new progress event supersedes an end.
  if (hypeTrainEndTimer) {
    clearTimeout(hypeTrainEndTimer);
    hypeTrainEndTimer = null;
  }
  renderHypeTrain(data);
  // On "end", keep the bar visible briefly (the final %) then hide.
  if (data.phase === "end") {
...
function addEventMessage(platform, kind, who, text, message, autoscroll, sharedFrom, sharedAvatar, badges) {
  ...
  // Shared Chat origin: prefer the origin streamer's avatar (round image),
  // fall back to a gold "↪ name" text badge when no avatar resolved.
  let sharedEl = null;
  if (sharedFrom) {
    if (sharedAvatar) {
      sharedEl = document.createElement("img");
      sharedEl.className = "source-streamer-avatar";
      sharedEl.src = sharedAvatar;
      sharedEl.alt = "";
      sharedEl.title = `${sharedFrom}'s chat`;
      sharedEl.loading = "lazy";
    } else {
      sharedEl = document.createElement("span");
      sharedEl.className = "shared-badge";
      sharedEl.textContent = `↪ ${sharedFrom}`;
    }
  }
  ...
  // Twitch badges (sub/founder/etc.) sit LEFT of the event text — badges
  // first, then the summary string. Using createTextNode so we don't
  // clobber the badges with the text content.
  if (platform === "tw") {
    const badgeFrag = buildTwitchBadges(badges);
    if (badgeFrag.childNodes.length) {
      label.appendChild(badgeFrag);
      label.appendChild(document.createTextNode(" "));
    }
  }
  label.appendChild(document.createTextNode(text || `${who || "Someone"} ${kind || ""}`));
  ...
  if (sharedEl) row.appendChild(sharedEl);
...
function eventIconFor(kind) {
  switch (kind) {
    ...
    case "raid":
      return "⚡"; // raid
    case "kicks":
      return "💎"; // Kick currency tip (Kicks = bits-equivalent)
    default:
      return "★";
  }
}
...
// sharedFrom (optional string): Twitch Shared Chat / extra Kick channel origin.
//   When sharedAvatar is also present we show the origin as a round avatar
//   image (Twitch Shared Chat); otherwise we fall back to a gold "↪ name" text
//   badge (Kick extra channels, or Twitch before/after GQL resolution).
// sharedAvatar (optional string): avatar URL for the Shared Chat origin streamer.
// badges (optional string[]): Twitch IRC badges (e.g. ["subscriber/12", "moderator/1"]).
function addChatMessage(platform, username, content, color, emotes, autoscroll, bits, sharedFrom, sharedAvatar, badges) {
  const wasAtBottom = autoscroll
    ? feed.scrollTop + feed.clientHeight >= feed.scrollHeight - 40
    : false;
  ...
  // Shared Chat origin: prefer the origin streamer's avatar (round image, with
  // the channel name in the tooltip). Fall back to a gold "↪ name" text badge
  // when no avatar resolved (Kick extra channels, or Twitch GQL still pending).
  let sharedEl = null;
  if (sharedFrom) {
    if (sharedAvatar) {
      sharedEl = document.createElement("img");
  ...
  // Twitch badges (sub/mod/vip/...) render LEFT of the username, matching
  // twitch.tv. We append badges first, then the name text — using
  // createTextNode instead of textContent so we don't clobber the badges.
  if (platform === "tw") {
    const badgeFrag = buildTwitchBadges(badges);
    if (badgeFrag.childNodes.length) {
      user.appendChild(badgeFrag);
  ...
  // cheered message reads "TW  [avatar]  ◆500  user: message".
...
//
// Three kinds of emote get rendered:
//   1) Native Twitch/Kick emotes — from the IRC `emotes` tag, with explicit
//      start/end ranges in the content (handled below).
//   2) Third-party Twitch emotes (FFZ/BTTV/7TV) — name→url map sent by the
//      server as {type:"emotes"}; we match whitespace-separated words against
//      it in the text BETWEEN (or instead of) native emotes.
function renderContentInto(el, content, emotes, platform) {
  content = String(content ?? "");
  const hasNative = Array.isArray(emotes) && emotes.length > 0;
  // Third-party emotes only apply to Twitch (Kick/YouTube have no FFZ/BTTV/7TV
  // integration). If the map is empty, fast-path to plain text.
  const hasThirdParty = platform === "tw" && thirdPartyEmotes.size > 0;
  if (!hasNative && !hasThirdParty) {
  ...
  for (const e of sorted) {
    ...
    if (e.start > cursor) {
      appendRichText(el, content.slice(cursor, e.start), hasThirdParty);
    }
  ...
  if (cursor < content.length) {
    appendRichText(el, content.slice(cursor), hasThirdParty);
    applied = true;
  }
  if (!applied) {
    // No native emote applied, but there may be third-party matches across the
    // whole string (the common case: a message with no native Twitch emotes).
    if (hasThirdParty) appendRichText(el, content, true);
    else el.textContent = content;
  }
}

// Appends a text slice to `el`, but splits it on whitespace and renders any
// word that matches a third-party emote name as an <img>. `enabled` lets the
// caller skip the word-scan when there's no third-party map.
function appendRichText(el, text, enabled) {
  if (!enabled) {
    el.appendChild(document.createTextNode(text));
    return;
  }
  // Split keeping the spaces so the layout is preserved. We scan each non-space
  // token; a match becomes an <img>, everything else stays text.
  ...
  for (const part of parts) {
    ...
    const url = thirdPartyEmotes.get(part);
    if (url) {
      el.appendChild(buildThirdPartyEmoteImg(url, part));
    } else {
      el.appendChild(document.createTextNode(part));
    }
  }
...
// Builds an <img> for a third-party emote (FFZ/BTTV/7TV). On error we fall back
// to the raw word so the message is never garbled. Mirrors buildEmoteImg.
function buildThirdPartyEmoteImg(url, name) {
  const img = document.createElement("img");
  img.className = "emote";
  img.src = url;
  img.alt = name;
...
function initTwitchAuthUI() {
  // If a token is saved, use it directly. The access token expires after ~1h,
  // so if the tab was closed overnight the stored token is almost certainly
  // dead. Proactively refresh it (silently) before trusting it — this covers
  // the common "authorized yesterday, came back today" case and avoids a
  // false "session expired" message on first load.
  const stored = readStoredTwitch();
  if (stored && stored.access_token) {
    twitchAccessToken = stored.access_token;
    twitchLogin = stored.login || "";
    setConnectedUI();
    // If the stored token is close to expiry (or we don't know when it
    // expires), renew now instead of trusting a possibly-stale token. Only
    // refresh once at boot; if it fails we fall back to disconnect UI.
    const stale = !stored.expires_at || stored.expires_at <= Date.now() + 5 * 60 * 1000;
    if (stale) {
      refreshTwitchToken().then((ok) => {
        if (ok) {
          scheduleTwitchRefresh(tokenExpiresAt);
        }
        // On failure refreshTwitchToken() already surfaces the right UI.
      });
    } else {
      scheduleTwitchRefresh(stored.expires_at);
    }
    return;
  }
  setDisconnectedUI();
...
function setDisconnectedUI() {
  twitchAccessToken = "";
  twitchLogin = "";
  // Stop any pending silent refresh and forget a half-finished retry.
  if (twitchRefreshTimer) {
    clearTimeout(twitchRefreshTimer);
    twitchRefreshTimer = null;
  }
  twitchRefreshPromise = null;
  pendingSendRetryContent = null;
...
// Schedule a silent token refresh ~1 min before expiry (or right now if it's
// already close). The server holds the refresh_token in an HttpOnly cookie, so
// this works even after a browser restart as long as the viewer stays logged in
// to our site. `expiresAtMs` = 0 means unknown → fall back to a sane default.
function scheduleTwitchRefresh(expiresAtMs) {
  if (twitchRefreshTimer) clearTimeout(twitchRefreshTimer);
  const REFRESH_LEAD_MS = 60 * 1000; // renew 1 min before expiry
  const DEFAULT_TTL_MS = 55 * 60 * 1000; // Twitch user tokens live ~1h
  let delay;
  if (expiresAtMs && expiresAtMs > Date.now()) {
    delay = expiresAtMs - Date.now() - REFRESH_LEAD_MS;
  } else {
    delay = DEFAULT_TTL_MS;
  }
  if (delay < 5000) delay = 5000; // never hammer the server
  twitchRefreshTimer = setTimeout(() => {
    twitchRefreshTimer = null;
    refreshTwitchToken().then((ok) => {
      if (ok) scheduleTwitchRefresh(tokenExpiresAt);
    });
  }, delay);
}

// Silently renew the Twitch access token. Returns true on success. On a hard
// failure (refresh token revoked) it drops the viewer back to "not connected".
// Deduped: concurrent callers share one in-flight promise.
function refreshTwitchToken() {
  if (twitchRefreshPromise) return twitchRefreshPromise;
  twitchRefreshPromise = (async () => {
    try {
      const r = await fetch("/api/twitch-refresh", {
        method: "POST",
        cache: "no-store",
      });
      if (!r.ok) return false;
      ...
      if (j.ok && j.access_token) {
        ...
        // The server returns an empty login (it doesn't store it per-user) —
        // keep the login we already have.
        tokenExpiresAt = j.expires_in > 0 ? Date.now() + j.expires_in * 1000 : 0;
        writeStoredTwitch(j.access_token, twitchLogin, tokenExpiresAt);
        return true;
      }
      // reauth:true means the refresh token is dead — the viewer must
      // re-authorize. Surface a message and reset the UI.
      if (j.reauth) {
        addSystemMessage("Your Twitch connection expired — please reconnect to keep chatting.");
        writeStoredTwitch("", "");
        setDisconnectedUI();
      }
      return false;
  ...
  return twitchRefreshPromise;
}

connectTwitchBtn.addEventListener("click", () => {
  if (twitchAccessToken) {
    // Disconnect: clear stored data, reset the UI, and tell the server to drop
    // the refresh cookie so it can no longer silently renew this account.
    writeStoredTwitch("", "");
    setDisconnectedUI();
    // Fire-and-forget — don't block the UI on the network round-trip.
    fetch("/api/twitch-disconnect", { method: "POST", cache: "no-store" }).catch(() => {});
    return;
  }
...
// Send a chat message. We remember the content so that if the server rejects
// the message because the token expired (the proactive refresh somehow missed
// its window), handleSendResult can silently refresh once and retry.
composerForm.addEventListener("submit", (e) => {
  ...
  pendingSendRetryContent = content; // remember for one-shot retry
  socket.send(
    JSON.stringify({
      type: "send",
...
function handleSendResult(msg) {
  ...
  // Token-shaped error: this is the "expires after ~1h" failure mode. Instead
  // of forcing the viewer to re-authorize, silently refresh once and retry the
  // exact message. Only fall back to disconnect if the refresh itself fails.
  const looksLikeTokenError = /not valid|invalid|unauthorized|token|expired/i.test(String(msg.error || ""));
  if (looksLikeTokenError) {
    const retryContent = pendingSendRetryContent;
    pendingSendRetryContent = null; // one-shot — never loop
    if (retryContent && socket && socket.readyState === WebSocket.OPEN) {
      refreshTwitchToken().then((ok) => {
        if (ok && twitchAccessToken) {
          // Retry the send with the fresh token. Disable the button again so
          // the viewer can't fire a duplicate while the retry is in flight.
          composerSend.disabled = true;
          socket.send(
            JSON.stringify({
              type: "send",
          ...
          addSystemMessage("Your Twitch session expired — please reconnect to keep chatting.");
          setDisconnectedUI();
        }
      });
      return;
