...
function parseKickFrame(raw) {
  ...
  // TEMP DIAGNOSTIC: focused logging to confirm gifted-sub events are arriving.
  // Log (a) any event whose name contains gift/lucky/leaderboard, and (b) every
  // event arriving on the channel.{id} public channel. This tells us whether
  // the issue is "event not arriving" vs "event arriving but parser wrong".
  const isGiftLike = /gift|lucky|leaderboard/i.test(eventName);
  const isOnChannel = typeof outer.channel === "string" && outer.channel.startsWith("channel.");
  if (isGiftLike || isOnChannel) {
    log.info({ event: eventName, channel: outer.channel, payload }, "kick gift/channel diagnostic");
  }
