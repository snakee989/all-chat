...
function parseKickFrame(raw) {
  ...
  // كيك يرفق مصفوفة emotes بكل رسالة (مع start/end ضمنية ضمن المحتوى)
  const emotes = parseKickEmotes(msgObj.emotes, content);

  if (!content) return null;
  return { id, username, content, color, emotes };
}

// يطبّع مصفوفة إيموت كيك ويصفي المداخل خارج النطاق.
function parseKickEmotes(rawEmotes, content) {
  ...
  for (const e of rawEmotes) {
    ...
    if (!Number.isFinite(id) || !Number.isFinite(start) || !Number.isFinite(end)) continue;
    if (start < 0 || end >= content.length || start > end) continue;
    out.push({ id, start, end });
  }
  return out;
