...
const CSP = [
  ...
  // The Twitch player iframe, plus alert-overlay iframes (StreamElements,
  // Streamlabs, etc.) which live on account-specific subdomains. We allow
  // https: broadly — same posture as img-src — since alert URLs are
  // user-supplied and we render them as hidden, sandboxed iframes.
  "frame-src https: ",
...
// Hype Train polling cadence. Twitch does not expose Hype Train over IRC; we
// poll its anonymous GQL endpoint. While a train is active we poll faster so
// the bar animates smoothly; while idle (no train) we poll slowly to conserve
// requests. Mirrors unified-chat-lite's cadence.
const HYPE_TRAIN_IDLE_POLL_MS = 60 * 1000;   // 60s when no train is active
const HYPE_TRAIN_ACTIVE_POLL_MS = 15 * 1000; // 15s while a train runs
// Max backoff after a failed GQL poll (doubles up to this cap).
const HYPE_TRAIN_MAX_BACKOFF_MS = 10 * 60 * 1000; // 10 minutes
...
// How long the server remembers that a session was destroyed because its
// stream ended (streamEnded=true). A returning client whose join matches a
// recorded key is sent `session_ended` instead of silently resuming, so the
// viewer re-enters channels manually rather than auto-rejoining a stale merged
// setup. 24h covers a "came back the next day" return; older markers are
// purged by the periodic sweep.
const ENDED_SESSION_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours
const KICK_PUSHER_APP_KEY = "32cbd69e4b950bf97679";

// Browser-like headers used for Kick's Cloudflare-fronted API. We send the full
// set a real Chrome tab would send (in a plausible order), not just User-Agent
// — Cloudflare increasingly fingerprints the whole header block, and a bare UA
// + Accept pair can be flagged as a bot. The reference impl (unified-chat-lite)
// goes further with curl_cffi's TLS impersonation; here we get most of the way
// with headers alone, which keeps the dep tree native. Keep these in sync with
// a current Chrome stable version when refreshing.
function kickBrowserHeaders(extra = {}) {
  return {
    "User-Agent":
...
const KICK_EVENT = {
  ...
  // Gifted subs arrive on the public channel.{channel_id} (NOT on
  // chatrooms.{id}.v2) as LuckyUsersWhoGotGiftSubscriptionsEvent. The payload
  // lists the recipients (user_ids/usernames); the gifter is in gifter_username
  // (live payload) or must be resolved from the companion GiftsLeaderboardUpdated
  // event. We also keep the older event names as fallbacks in case Kick's older
  // revisions still fire them.
  GIFTED_SUBS: "App\\Events\\LuckyUsersWhoGotGiftSubscriptionsEvent",
  GIFTED_SUBS_LEGACY: "App\\Events\\GiftedSubscriptionsEvent",
  GIFTS_LEADERBOARD: "App\\Events\\GiftsLeaderboardUpdated",
  ...
  // "Kicks" is Kick's virtual currency (the bits-equivalent). When a viewer
  // tips Kicks to a streamer, Kick fires this event on the public
  // channel.{channel_id} (NOT on chatrooms.{id}.v2). Note the bare name with
  // no "App\Events\" prefix — Kick uses this short form for currency tips.
...
// Twitch's anonymous web-client GQL endpoint + client id. Both are the same
// ones twitch.tv itself uses; GQL accepts these without any OAuth, so we can
// resolve Shared Chat origin channels (login + avatar) without relying on
// TWITCH_CLIENT_ID/SECRET being configured. Mirrors unified-chat-lite's approach.
const TWITCH_GQL_URL = "https://gql.twitch.tv/gql";
const TWITCH_GQL_CLIENT_ID = "kimne78kx3ncx6brgo4mv6wki5h1ko";

// Explicit, modest body-size limits: our JSON bodies are tiny (auth/join/send),
// so 16 KiB is more than enough and rejects oversized payloads early. (Express
// 4 defaults to 100 KiB; being explicit protects against any future change and
...
const authLimiter = rateLimit({
  ...
  // We run behind Cloudflare/nginx and intentionally trust X-Forwarded-For
  // (see app.set("trust proxy") above). Silence the library's permissive-proxy
  // validation so the limiters actually run instead of throwing on each request.
...
// Normalizes and validates a chat handle (Twitch / Kick / TikTok username).
// Returns the cleaned lowercase handle, or null if the input is empty/invalid.
// Twitch/Kick/TikTok logins are alphanumeric + underscore, 3–25 chars; we
// accept the slightly looser [A-Za-z0-9_] and lowercase it. This rejects:
//   - oversized handles (a malicious client pushing a 100 KB string into the
//     session key / IRC JOIN / Helix URL),
//   - control chars and whitespace (which could be smuggled into IRC),
//   - obviously-junk values before we ever open a platform connection.
// YouTube is NOT routed through here — it accepts full URLs, handled in join().
function normalizeChatHandle(value) {
  if (typeof value !== "string") return null;
  const v = value.trim().toLowerCase();
  if (!v) return null;
  // Allow the realistic login alphabet only. Reject anything longer than 25
  // chars or containing chars outside it.
...
// ----------------------------------------------------------------------
// Kick fetch: Cloudflare on kick.com fingerprints the TLS handshake
// (JA3/JA4) in addition to inspecting HTTP headers. Node's fetch (undici)
// uses OpenSSL, whose fingerprint differs from a real Chrome's (BoringSSL),
// so kick.com's WAF returns 403 "Request blocked by security policy." for
// every Node-originated request — even with a perfect Chrome header set.
//
// Working around this from inside Node requires a TLS-impersonating library
// (curl_cffi / node-libcurl with a patched libcurl), which is heavy and
// platform-specific. Instead we delegate Kick requests to the system `curl`,
// which uses the OS's native TLS stack (Schannel on Windows, NSS/GnuTLS on
// Linux) and therefore has a DIFFERENT fingerprint that Cloudflare does not
// block. This was verified empirically: identical headers via Node fetch ->
// 403; via curl -> 200.
//
// Strategy: try Node fetch first (fast, works where CF is not blocking), and
// fall back to curl only on a 403 (the WAF signal). This keeps normal latency
// low and avoids a process spawn per request when fetch works. If curl is not
// installed we surface the original fetch error.
// ----------------------------------------------------------------------
...
function isCurlAvailable() {
  ...
  curlChecked = true;
  try {
    // Quick synchronous probe: `curl --version` exits 0 if curl is on PATH.
    execFile("curl", ["--version"], { stdio: "ignore", timeout: 2000 }, (err) => {
      // Async result updates the cached flag; first request may miss it but
      // subsequent ones benefit. Re-checked if curl was absent on the probe.
      if (!err) {
        curlAvailable = true;
      } else {
        curlAvailable = false;
        log.warn({ err: String(err && err.message ? err.message : err) }, "curl not found; Kick will rely on Node fetch (may be 403-blocked by Cloudflare)");
      }
    });
    // Optimistically assume present until the probe says otherwise; this
    // avoids dropping the very first Kick request right after process start.
    curlAvailable = true;
  } catch {
    curlAvailable = false;
  }
...
// Performs a Kick API GET via the system curl. Resolves with the parsed JSON
// body, or throws on non-2xx / curl failure. Mirrors the fetchWithTimeout
// contract so callers don't change shape.
function kickFetchViaCurl(url, timeoutMs = FETCH_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    const args = [
      ...
      // Ask curl to request the same compressed encodings a browser would,
      // then auto-decompress (--compressed). Important: this also keeps the
      // Accept-Encoding header order stable, which CF partly fingerprints.
      "--compressed",
      url,
    ];
    execFile("curl", args, { maxBuffer: 5 * 1024 * 1024, timeout: timeoutMs + 2000 }, (err, stdout, stderr) => {
      if (err) {
        // curl exits non-zero on network/timeout failures. Surface a clean
        // Error so the caller's catch path behaves like a fetch failure.
        const detail = String(stderr || err.message || err).trim();
        return reject(new Error(`curl failed: ${detail || err.code || "unknown"}`));
      }
      // We did not pass -w, so stdout is the raw body. Detect Cloudflare blocks
      // by content rather than exit code (curl returns 0 for a 403 body).
      let body = stdout;
      // Parse so we can read the embedded HTTP "error" field Kick/CF returns.
      let parsed;
      try {
        parsed = JSON.parse(body);
      } catch {
        return reject(new Error("Kick returned a non-JSON body"));
      }
      // Kick's CF block looks like {"error":"Request blocked by security policy."}
      // with no HTTP status exposed via curl (curl hides it by default). We
      // detect it by the known error string and reject so the caller treats it
      // as a failed request (state="unknown"), not a successful empty result.
      if (parsed && typeof parsed.error === "string" && /security policy|blocked/i.test(parsed.error)) {
        return reject(new Error("Kick blocked the request (Cloudflare security policy)"));
      }
      resolve(parsed);
    });
  });
...
// High-level Kick fetch: tries Node fetch first; on a 403 (Cloudflare WAF),
// retries via curl which has a different TLS fingerprint. Returns the parsed
// JSON, or throws. Used by lookupKickChatroom and fetchKickViewerCount.
async function kickFetchJson(urlPath, timeoutMs = FETCH_TIMEOUT_MS) {
  const url = urlPath.startsWith("http") ? urlPath : `https://kick.com${urlPath}`;
  // First attempt: native fetch.
  let lastErr;
  try {
    const r = await fetchWithTimeout(url, { headers: kickBrowserHeaders() }, timeoutMs);
    if (r.status === 403) {
      // Cloudflare fingerprint block — try curl before giving up.
      lastErr = new Error("Kick rejected the request (403)");
    } else if (r.status === 404) {
      const e = new Error("This user was not found on Kick");
      e.status = 404;
      throw e;
    } else if (!r.ok) {
      throw new Error(`Kick rejected the request (${r.status})`);
    } else {
      return await r.json();
    }
  } catch (err) {
    // Network error / abort — remember it and try curl if available.
    if (err && err.status === 404) throw err; // 404 is deterministic; don't retry
    lastErr = err;
  }
  // Fallback: curl (different TLS fingerprint, bypasses the WAF).
  if (isCurlAvailable()) {
    try {
      return await kickFetchViaCurl(url, timeoutMs);
    } catch (err) {
      // curl failed too — prefer the more informative original error.
      throw lastErr || err;
    }
  }
...
// Records sessions destroyed because their stream ended (streamEnded=true),
// keyed by the same session key. A returning client whose join matches a key
// here is sent `session_ended` instead of silently resuming, so the viewer
// re-enters channels manually. Entries are one-shot (deleted on first match)
// and also purged past ENDED_SESSION_TTL_MS by the periodic sweep.
...
class Session {
  constructor(twitchUser, kickUser, tiktokUser, youtubeInput) {
    ...
    this.platforms = {
      twitch: {
        ...
        // Hype Train (Twitch). Polled via anonymous GQL while the channel is
        // live. `hypeTrain` holds the last broadcasted state (so a reconnecting
        // client can re-render it from the snapshot); `hypeTimer` is the poll
        // handle. Cleared on teardown/reconnect.
        hypeTrain: null, hypeTimer: null,
        // Third-party emotes (FFZ/BTTV/7TV). Fetched once after the channel
        // joins, then broadcast to clients as {type:"emotes", emotes:{name:url}}.
        // `emotes` holds the last fetched map so a reconnecting client gets it
        // from the snapshot. `emotesTask` tracks the in-flight fetch (cancelled
        // on teardown).
        emotes: null, emotesTask: null,
      },
      kick: {
  ...
  addClient(ws, lastId) {
    ...
    ws.send(
      JSON.stringify({
        ...
        // Current Hype Train state (null when none). Sent so a reconnecting
        // client can re-render the bar without waiting for the next poll tick.
        hypeTrain: this.platforms.twitch.hypeTrain,
        // Third-party emotes map (null while loading / when Twitch isn't
        // configured). Sent so a reconnecting client can render KEKW/monkaS/…
        // without waiting for the next fetch.
        emotes: this.platforms.twitch.emotes,
        backlog,
        lastId: this.seq,
  ...
  scheduleExpiry() {
    ...
    this.graceTimer = setTimeout(() => {
      ...
      // If the stream was confirmed ended, record a marker so a returning client
      // is sent to the setup screen instead of silently resuming a stale merged
      // session. A plain grace expiry (stream still live) records nothing.
      if (this.streamEnded) {
        endedSessions.set(this.key, { exp: Date.now() + ENDED_SESSION_TTL_MS });
        log.info({ session: this.key }, "recording stream-ended marker");
      }
      this.destroy();
  ...
  // sharedAvatar: optional string — avatar URL for the origin channel (Twitch
  //   Shared Chat only). When present the client renders an avatar image
  //   instead of the gold "↪ name" text badge.
  pushChat(platform, username, content, color, emotes, bits, sharedFrom, sharedAvatar, badges) {
    const msg = {
    ...
    if (Array.isArray(badges) && badges.length) msg.badges = badges;
    if (sharedFrom) {
      msg.sharedFrom = sharedFrom;
      if (sharedAvatar) msg.sharedAvatar = sharedAvatar;
    }
  ...
  pushEvent(platform, kind, who, text, info) {
    ...
    if (info && info.sharedFrom) {
      ...
      if (info.sharedAvatar) msg.sharedAvatar = info.sharedAvatar;
    }
    if (info && Array.isArray(info.badges) && info.badges.length) {
      msg.badges = info.badges; // Twitch sub/mod/vip/etc. badges on the actor
    }
    this.backlog.push(msg);
    while (this.backlog.length > BACKLOG_LIMIT) this.backlog.shift();
    this.broadcast(msg);
...
async function lookupKickChatroom(username) {
  // kickFetchJson tries Node fetch first, then falls back to the system curl
  // (different TLS fingerprint) on a 403 Cloudflare WAF block. This used to
  // call fetchWithTimeout directly, which Cloudflare now rejects with 403 for
  // every Node request because of JA3/JA4 TLS fingerprinting.
  const data = await kickFetchJson(`/api/v2/channels/${encodeURIComponent(username)}`);
  const chatroomId = data?.chatroom?.id;
  if (!chatroomId) throw new Error("This user has no chat (they may never have streamed)");
  // The channel_id (data.id) is distinct from the chatroom_id and is needed to
  // subscribe to the public presence channel (channel.{id}), which carries
  // events that the chatrooms.{id}.v2 channel does not (e.g. gifted subs).
  const channelId = Number(data?.id) || null;
  // profile_pic of the streamer — used as the avatar for mirrored Kick channels
  // (the gold "↪ name" badge is upgraded to the channel's profile image, same
  // as we do for Twitch Shared Chat). Empty when Kick omits it.
  const avatar = data?.user?.profile_pic || data?.profile_pic || "";
  return { chatroomId, channelId, avatar };
}
...
app.get("/auth/twitch", oauthLimiter, (req, res) => {
  ...
  const params = new URLSearchParams({
    ...
    response_type: "code",
    // NOTE: Twitch issues a refresh_token automatically with the Authorization
    // Code flow — it does NOT use (and rejects) an `offline_access` scope like
    // Discord/Azure do. We capture that refresh_token server-side and use it to
    // silently renew the access token before it expires (~1h).
...
app.get("/auth/twitch/callback", oauthLimiter, async (req, res) => {
  ...
  // Persist the refresh_token (if Twitch returned one) in a long-lived HttpOnly
  // cookie on the server. Twitch issues a refresh_token automatically with the
  // Authorization Code flow (no offline_access scope needed — that's a
  // Discord/Azure convention; see the note above /auth/twitch). We never expose
  // the refresh_token to JS; the client asks POST /api/twitch-refresh to renew.
  // Twitch rotates refresh tokens, so this cookie is updated on every refresh.
  const refreshToken = typeof tokenJson.refresh_token === "string" ? tokenJson.refresh_token : "";
  if (refreshToken) {
    setAuthCookie(res, "tw_refresh", refreshToken, 365 * 24 * 60 * 60); // ~1 year
  }
  // expires_in (seconds) tells the client when to renew proactively.
  const expiresIn = Number(tokenJson.expires_in) > 0 ? Number(tokenJson.expires_in) : 0;
  ...
  oauthTickets.set(ticket, { token: accessToken, login, expiresIn, exp: Date.now() + 30000 });
  ...
  log.info({ login: login || "(unknown)", hasRefresh: !!refreshToken }, "oauth success");
  res.redirect(302, "/");
});
...
app.get("/api/twitch-token", apiLimiter, (req, res) => {
  ...
  res.json({ ok: true, access_token: data.token, login: data.login || "", expires_in: data.expiresIn || 0 });
});

// ----------------------------------------------------------------------
// Twitch token refresh: renews an expired (or soon-to-expire) access token
// using the refresh_token stored in the HttpOnly `tw_refresh` cookie. The
// refresh_token itself never reaches the browser. Twitch rotates refresh
// tokens, so on success we overwrite the cookie with the new one.
// Called silently by the client ~1 min before the access token expires so a
// viewer never has to re-authorize mid-stream.
// ----------------------------------------------------------------------
// In-flight refresh dedupe. Twitch rotates refresh tokens on every refresh, so
// if two tabs (or the proactive timer + an on-load refresh) call /refresh at the
// same time with the same old token, Twitch sees the second as reuse of a
// revoked token and revokes the whole chain. Key by the exact cookie value and
// share the single in-flight request across concurrent callers.
const inflightRefreshes = new Map(); // refreshToken -> Promise<result>

app.post("/api/twitch-refresh", apiLimiter, async (req, res) => {
  // This response carries a fresh access_token — never cache it.
  res.setHeader("Cache-Control", "no-store");
  const refreshToken = readAuthCookie(req, "tw_refresh");
  if (!refreshToken || !TWITCH_CLIENT_ID || !TWITCH_CLIENT_SECRET) {
    return res.json({ ok: false, reauth: true });
  }
  // Dedupe: if a refresh for this exact token is already in flight, wait on it
  // instead of firing a second Twitch request that would reuse+revoke the token.
  if (!inflightRefreshes.has(refreshToken)) {
    inflightRefreshes.set(refreshToken, doTwitchRefresh(refreshToken));
    inflightRefreshes.get(refreshToken).finally(() => inflightRefreshes.delete(refreshToken));
  }
  const result = await inflightRefreshes.get(refreshToken);
  if (result.clearCookie) clearAuthCookie(res, "tw_refresh");
  if (result.setCookie) setAuthCookie(res, "tw_refresh", result.refreshToken, 365 * 24 * 60 * 60);
  res.json(result.body);
});

// Perform the actual token refresh against Twitch. Returns a plain object the
// route handler translates into HTTP (cookie writes + JSON body). Isolated so it
// can be deduped across concurrent requests without per-request Twitch calls.
async function doTwitchRefresh(refreshToken) {
  let r;
  try {
    r = await fetchWithTimeout("https://id.twitch.tv/oauth2/token", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({
        client_id: TWITCH_CLIENT_ID,
    ...
    log.warn({ err }, "twitch token refresh network error");
    return { body: { ok: false, reauth: false, retry: true } };
  }
  if (!r.ok) {
    // 400/401 from Twitch = the refresh token is invalid/revoked/used. The
    // viewer must re-authorize. Clear the dead cookie so we stop retrying.
    log.warn({ status: r.status }, "twitch token refresh rejected");
    return { clearCookie: true, body: { ok: false, reauth: true } };
  }
  ...
  // Persist the (possibly rotated) refresh token for next time. Twitch rotates
  // refresh tokens, so the new value must overwrite the old cookie.
  const newRefresh = typeof j.refresh_token === "string" ? j.refresh_token : "";
  const expiresIn = Number(j.expires_in) > 0 ? Number(j.expires_in) : 0;
  // We don't have the login cached server-side per user, but the client
  // already knows it; returning an empty login tells it to keep the existing.
  return {
    setCookie: !!newRefresh,
    refreshToken: newRefresh,
    body: { ok: true, access_token: accessToken, login: "", expires_in: expiresIn },
  };
}

// Twitch disconnect: clears the server-side refresh cookie so the browser can
// no longer silently renew. Called when the viewer clicks "Disconnect".
app.post("/api/twitch-disconnect", apiLimiter, (req, res) => {
  clearAuthCookie(res, "tw_refresh");
  res.json({ ok: true });
});
...
// `source-room-id` tag) to its login + profile avatar so the client can label
// mirrored messages with the origin streamer's avatar (and name in the
// tooltip). Uses Twitch's anonymous GQL endpoint (the same one twitch.tv
// uses; accepts the hardcoded web-client id without OAuth) — this is what
// unified-chat-lite does, and unlike Helix it needs no client credentials,
// so Shared Chat works even on a server with no TWITCH_CLIENT_ID/SECRET.
// Results are cached for the process lifetime. On failure, resolution
// silently no-ops (the client then shows a generic "↪ shared" badge).
// ----------------------------------------------------------------------
const channelIdToInfo = new Map();                        // source room id -> { login, avatar } (permanent cache)
const pendingChannelLookups = new Map();                  // source room id -> in-flight Promise (dedup)

// Cached Twitch app access token (server-to-server, used for Helix /streams
// viewer counts and any other app-level call). { token, exp } or null.
// NOTE: this MUST be declared before getTwitchAppAccessToken() reads it — the
// previous code referenced `twitchAppToken` without ever declaring it, which
// threw ReferenceError on first use and silently broke Twitch viewer polling
// plus stream-ended detection.
...
async function getTwitchAppAccessToken() {
  ...
  if (twitchAppToken && twitchAppToken.token && twitchAppToken.exp - 60000 > Date.now()) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return twitchAppToken ? twitchAppToken.token : null; // keep the previous token (if any) rather than clearing
    }
    const j = await r.json();
    if (!j.access_token || !j.expires_in) {
      log.warn("twitch app token response missing fields");
      return twitchAppToken ? twitchAppToken.token : null;
    }
...
// Resolves a Twitch channel id to { login, avatar } via anonymous GQL.
// Cached permanently per process; concurrent calls for the same id coalesce
// into one in-flight request. Returns null on any failure (caller falls back
// to a generic "↪ shared" badge).
async function resolveChannelLogin(channelId) {
  if (!channelId) return null;
  const cached = channelIdToInfo.get(channelId);
  ...
  const p = (async () => {
    try {
      const r = await fetchWithTimeout(TWITCH_GQL_URL, {
        method: "POST",
      ...
      if (!r.ok) {
        log.warn({ status: r.status, channelId }, "twitch gql channel lookup rejected");
        return null;
      }
      const j = await r.json();
      ...
      channelIdToInfo.set(channelId, info);
      return info;
    } catch (err) {
      log.warn({ err: String(err?.message || err), channelId }, "twitch gql channel lookup failed");
      return null;
    }
  })();
...
function readAuthCookie(req, name) {
  ...
  for (const part of header.split(";")) {
    ...
    if (k === name) {
      // decodeURIComponent throws URIError on malformed %-sequences (a client
      // or proxy can send one). Treat that as "no cookie" rather than letting
      // it 500 the whole request — the caller already handles a missing cookie.
      try {
...
function connectTwitch(session) {
  ...
  socket.addEventListener("message", async (event) => {
    ...
    for (const line of lines) {
      ...
      if (parsed.joined) {
        ...
        // Also begin the Hype Train poll (anonymous GQL). Same lifecycle.
        startHypeTrainPoll(session);
        // Kick off the third-party emotes (FFZ/BTTV/7TV) fetch in the
        // background — broadcast to clients once it resolves. Idempotent: if
        // already loaded for this channel, it just re-broadcasts.
        loadThirdPartyEmotes(session);
        continue;
      }
      if (parsed.message) {
        // Emit immediately for non-shared messages. For Shared Chat messages
        // (carrying a source-room-id) we resolve the origin channel login +
        // avatar in the BACKGROUND (non-blocking) so a slow GQL lookup can
        // never stall the rest of the feed. Message ordering across the
        // boundary is not critical because the client renders by monotonic `id`
        // and the origin only affects the avatar badge.
        ...
        if (m.sharedRoomId) {
          resolveChannelLogin(m.sharedRoomId)
            .then((info) => {
              if (session.closed) return;
              session.pushChat(
                "tw", m.username, m.content, m.color, m.emotes, m.bits,
                info?.login || "shared", info?.avatar || "", m.badges
              );
            })
            .catch(() => {
              if (session.closed) return;
              session.pushChat("tw", m.username, m.content, m.color, m.emotes, m.bits, "shared", "", m.badges);
            });
        } else {
          session.pushChat("tw", m.username, m.content, m.color, m.emotes, m.bits, null, "", m.badges);
        }
        continue;
      }
      if (parsed.event) {
        ...
        if (ev.sharedRoomId) {
          resolveChannelLogin(ev.sharedRoomId)
            .then((info) => {
              if (session.closed) return;
              ev.sharedFrom = info?.login || "shared";
              if (info?.avatar) ev.sharedAvatar = info.avatar;
  ...
  socket.addEventListener("close", () => {
    ...
    // Same for the Hype Train poll — restarts on the next live JOIN.
    stopHypeTrainPoll(session);
    if (session.closed) return;
...
// ----------------------------------------------------------------------
// Twitch Hype Train: polled via anonymous GQL (same endpoint as Shared Chat
// avatar resolution). No OAuth needed. We poll slowly while no train is active
// and quickly while one runs so the bar animates smoothly. Fully firewalled:
// any failure logs and backs off; nothing else is affected. Mirrors
// unified-chat-lite's HypeTrainPoller.
// ----------------------------------------------------------------------
const HYPE_TRAIN_GQL_QUERY =
  "query($login: String){user(login: $login){channel{hypeTrain{execution{isActive progress{goal progression level{value}}}}}}}";

// Fetches the current Hype Train state for a Twitch login via anonymous GQL.
// Returns null when no train is active (or the channel/user resolves but has
// no hypeTrain field). Throws on a malformed response shape so the caller
// backs off; returns {level, progress, goal} when a train is running.
async function fetchTwitchHypeTrain(login) {
  const r = await fetchWithTimeout(TWITCH_GQL_URL, {
    method: "POST",
    headers: { "Client-ID": TWITCH_GQL_CLIENT_ID, "Content-Type": "application/json" },
    body: JSON.stringify({ query: HYPE_TRAIN_GQL_QUERY, variables: { login } }),
  });
  if (!r.ok) throw new Error(`gql rejected (${r.status})`);
  const j = await r.json();
  const channel = j?.data?.user?.channel;
  if (!channel || typeof channel !== "object" || !("hypeTrain" in channel)) {
    throw new Error("unexpected gql response shape");
  }
  const exec = channel.hypeTrain?.execution;
  if (!exec || !exec.isActive) return null;
  const progress = exec.progress || {};
  const level = Number(progress.level?.value);
  const progression = Number(progress.progression);
  const goal = Number(progress.goal);
  return {
    level: Number.isFinite(level) && level > 0 ? level : 1,
    progress: Number.isFinite(progression) && progression >= 0 ? progression : 0,
    goal: Number.isFinite(goal) && goal > 0 ? goal : 0,
  };
}

// Starts the Hype Train poll loop for this session's Twitch channel. Self-
// schedules the next tick (the cadence varies by activity), so it is NOT a
// setInterval — we clear the per-tick timer each round. Idempotent: if a poll
// is already running this is a no-op.
function startHypeTrainPoll(session) {
  const st = session.platforms.twitch;
  if (st.hypeTimer || session.closed) return;
  const tick = async () => {
    if (session.closed) return;
    // Only poll while the IRC read connection is live; the close handler stops
    // the poll on disconnect, but a status change between ticks could leave us
    // polling a dead session otherwise.
    if (st.status !== "live") {
      st.hypeTimer = setTimeout(tick, HYPE_TRAIN_IDLE_POLL_MS);
      return;
    }
    let delay = HYPE_TRAIN_IDLE_POLL_MS;
    try {
      const train = await fetchTwitchHypeTrain(session.twitchUser);
      if (session.closed) return;
      if (train) {
        // Active train: store + broadcast the updated state and keep polling fast.
        delay = HYPE_TRAIN_ACTIVE_POLL_MS;
        const prev = st.hypeTrain;
        const changed =
          !prev || prev.phase !== "progress" ||
          prev.level !== train.level || prev.progress !== train.progress ||
          prev.goal !== train.goal;
        if (changed) {
          st.hypeTrain = { phase: "progress", ...train };
          session.broadcast({ type: "hype_train", ...st.hypeTrain });
        }
      } else {
        // No train: if we previously showed one, broadcast its end then clear.
        delay = HYPE_TRAIN_IDLE_POLL_MS;
        if (st.hypeTrain) {
          st.hypeTrain = null;
          session.broadcast({
            type: "hype_train",
      ...
      log.warn({ err: String(err?.message || err), channel: session.twitchUser }, "twitch hype train poll failed");
      // Exponential backoff on failure, capped.
      delay = Math.min(Math.max(delay, HYPE_TRAIN_IDLE_POLL_MS) * 2, HYPE_TRAIN_MAX_BACKOFF_MS);
    }
    if (!session.closed) st.hypeTimer = setTimeout(tick, delay);
  };
  // Fire the first poll immediately so a train already in progress appears at
  // connect, then it self-schedules.
  st.hypeTimer = setTimeout(tick, 0);
}

function stopHypeTrainPoll(session) {
  const st = session.platforms.twitch;
  if (st.hypeTimer) {
    clearTimeout(st.hypeTimer);
    st.hypeTimer = null;
  }
}

// Loads FFZ/BTTV/7TV emotes for this session's Twitch channel in the
// background (non-blocking — a slow fetch never stalls the feed). On resolve,
// caches the map on the session and broadcasts it to all clients. Idempotent:
// re-broadcasts the cached map if called again for the same channel. Cleared
// on teardown via `emotesTask = null`.
function loadThirdPartyEmotes(session) {
  if (session.closed) return;
  const st = session.platforms.twitch;
  if (st.emotesTask) return; // already loading
  if (st.emotes) {
    // Already loaded — re-broadcast (covers a fresh client joining late).
    session.broadcast({ type: "emotes", emotes: st.emotes });
    return;
  }
  st.emotesTask = (async () => {
    const emotes = await fetchChannelThirdPartyEmotes(session.twitchUser);
    if (session.closed) return;
    st.emotes = emotes;
    st.emotesTask = null;
    session.broadcast({ type: "emotes", emotes });
    log.info({ session: session.key, channel: session.twitchUser, count: Object.keys(emotes).length }, "third-party emotes loaded");
  })().catch((err) => {
    st.emotesTask = null;
    log.warn({ err: String(err?.message || err), channel: session.twitchUser }, "third-party emotes load failed");
  });
}

// ----------------------------------------------------------------------
// Third-party Twitch emotes (FFZ / BTTV / 7TV). Twitch's IRC `emotes` tag
// only carries native Twitch emotes — the popular ones viewers actually use
// (KEKW, monkaS, pepeD, …) come from FFZ/BTTV/7TV and arrive as plain text.
// We fetch the per-channel + global emote sets once (public APIs, no keys),
// build a name→image-url map, and broadcast it to the client so the renderer
// can swap matching words for <img>. Mirrors unified-chat-lite's emotes.py.
// Fully firewalled: any failure logs and returns {} — chat still works.
// ----------------------------------------------------------------------
const THIRD_PARTY_EMOTE_TIMEOUT_MS = 15000;
let globalEmotesCache = null; // { name: url } — fetched once per process
let globalEmotesPromise = null; // dedup concurrent global fetches

async function fetchJsonWithTimeout(url) {
  const r = await fetchWithTimeout(url, { timeout: THIRD_PARTY_EMOTE_TIMEOUT_MS });
  if (!r.ok) return null;
  return r.json();
}

// Global emote sets (FFZ/BTTV/7TV) shared across every channel. Fetched once
// and cached for the process lifetime; concurrent callers share one fetch.
async function fetchGlobalThirdPartyEmotes() {
  ...
  globalEmotesPromise = (async () => {
    ...
    // FFZ global set.
    try {
      const ffz = await fetchJsonWithTimeout("https://api.frankerfacez.com/v1/set/global");
      if (ffz) Object.assign(out, parseFfzEmotes(ffz));
    } catch (err) { log.warn({ err: String(err?.message || err) }, "ffz global emotes failed"); }
    // BTTV global emotes (returns an array).
    try {
      const bttv = await fetchJsonWithTimeout("https://api.betterttv.net/3/cached/emotes/global");
      if (Array.isArray(bttv)) Object.assign(out, parseBttvEmotes(bttv));
    } catch (err) { log.warn({ err: String(err?.message || err) }, "bttv global emotes failed"); }
    // 7TV global set.
    try {
      const stv = await fetchJsonWithTimeout("https://7tv.io/v3/emote-sets/global");
      if (stv) Object.assign(out, parseSevenTvEmotes(stv.emotes || []));
    } catch (err) { log.warn({ err: String(err?.message || err) }, "7tv global emotes failed"); }
    globalEmotesCache = out;
    return out;
  })().catch((err) => {
    log.warn({ err: String(err?.message || err) }, "third-party global emotes fetch failed");
    globalEmotesCache = {}; // cache an empty map so we don't keep retrying
...
function parseFfzEmotes(data) {
  const out = {};
  for (const set of Object.values(data?.sets || {})) {
    for (const e of set.emoticons || []) {
      const name = e.name;
      const urls = e.urls || {};
      let url = urls["2"] || urls["1"] || "";
...
function parseBttvEmotes(items) {
  const out = {};
  for (const item of items) {
    if (item?.code && item?.id) {
      out[item.code] = `https://cdn.betterttv.net/emote/${item.id}/2x`; // 2x = animated-capable, crisp
    }
  }
  return out;
}

function parseSevenTvEmotes(items) {
  const out = {};
  for (const item of items) {
    const name = item?.name;
    const id = item?.id;
    if (name && id) out[name] = `https://cdn.7tv.app/emote/${id}/2x.webp`; // 2x.webp = animated + crisp
  }
  return out;
}

// Per-channel third-party emotes (channel-specific + shared). Looks up the
// Twitch channel id via FFZ's room endpoint (it returns twitch_id), then
// fetches BTTV/7TV channel emotes. Combined with the global set. Cached per
// channel for the process lifetime.
const channelEmotesCache = new Map(); // login -> { name: url }
const channelEmotesPending = new Map(); // login -> in-flight Promise (dedup)

async function fetchChannelThirdPartyEmotes(login) {
  const cached = channelEmotesCache.get(login);
  if (cached) return cached;
  const pending = channelEmotesPending.get(login);
  if (pending) return pending;
  const p = (async () => {
    // Start from the globals so the client gets one merged map per channel.
    const out = { ...(await fetchGlobalThirdPartyEmotes()) };
    // FFZ room gives us the twitch_id we need for BTTV/7TV.
    let twitchId = null;
    try {
      const ffz = await fetchJsonWithTimeout(`https://api.frankerfacez.com/v1/room/${encodeURIComponent(login)}`);
      if (ffz) {
        Object.assign(out, parseFfzEmotes(ffz));
        twitchId = ffz?.room?.twitch_id || null;
      }
    } catch (err) { log.warn({ err: String(err?.message || err), login }, "ffz channel emotes failed"); }
    if (twitchId) {
      try {
        const bttv = await fetchJsonWithTimeout(`https://api.betterttv.net/3/cached/users/twitch/${twitchId}`);
        if (bttv) {
          Object.assign(out, parseBttvEmotes((bttv.channelEmotes || []).concat(bttv.sharedEmotes || [])));
        }
      } catch (err) { log.warn({ err: String(err?.message || err), login }, "bttv channel emotes failed"); }
      try {
        const stv = await fetchJsonWithTimeout(`https://7tv.io/v3/users/twitch/${twitchId}`);
        if (stv) Object.assign(out, parseSevenTvEmotes(stv?.emote_set?.emotes || []));
      } catch (err) { log.warn({ err: String(err?.message || err), login }, "7tv channel emotes failed"); }
    }
    channelEmotesCache.set(login, out);
    return out;
  })().catch((err) => {
    log.warn({ err: String(err?.message || err), login }, "third-party channel emotes fetch failed");
    channelEmotesCache.set(login, {});
    return channelEmotesCache.get(login);
  }).finally(() => {
    channelEmotesPending.delete(login);
  });
  channelEmotesPending.set(login, p);
  return p;
}
...
function teardownTwitch(session) {
  ...
  stopHypeTrainPoll(session);
  st.hypeTrain = null;
  // Drop the third-party emotes fetch (no cancel needed — it self-clears on
  // resolve). We keep the cached map so a reconnecting client still gets it
  // from the snapshot, but a fresh JOIN re-broadcasts it.
  st.emotesTask = null;
  st.reconnecting = false;
  if (st.socket) {
    try {
...
function parseTwitchLine(line) {
  ...
  if (privmsgMatch) {
    ...
    if (badges.length) out.message.badges = badges;
  ...
  if (usernoticeMatch) {
    ...
    const badges = parseTwitchBadges(tags["badges"]);
    const event = parseUsernotice(tags, prefix, customMsg, sharedRoomId, badges);
...
// Parses the IRC `badges` tag (e.g. "subscriber/12,moderator/1,vip/1") into an
// array of ["subscriber/12", "moderator/1", ...] preserving order. The client
// maps each badge slug to a CDN image (static-cdn.jtvnw.net). We forward the
// raw "slug/version" form verbatim — the version matters for channel-specific
// sub badges, but only the slug is needed for the global ones we render.
function parseTwitchBadges(rawBadges) {
  if (!rawBadges) return [];
...
function parseUsernotice(tags, prefix, customMsg, sharedRoomId, badges) {
  ...
  if (Array.isArray(badges) && badges.length) event.badges = badges;
  ...
  for (const channelName of session.kickUsers) {
    if (!session.platforms.kick.conns.has(channelName)) {
      session.platforms.kick.conns.set(channelName, {
        socket: null,
        chatroomId: null,
        channelId: null, // distinct from chatroomId; used for the public channel.{id} subscription
        avatar: "",      // streamer profile_pic, used as the shared-chat avatar for mirrors
        pingTimer: null,
        reconnectTimer: null,
        reconnecting: false,
        status: "connecting",
...
async function connectKickChannel(session, channelName) {
  ...
  // Resolve chatroom_id + channel_id (+ streamer avatar) once and reuse across
  // reconnects. The avatar is only consumed by mirror channels (rendered as
  // their shared-chat badge), but we fetch it for the primary too since the
  // lookup happens once regardless.
  if (!conn.chatroomId) {
    try {
      const ids = await lookupKickChatroom(channelName);
      conn.chatroomId = ids.chatroomId;
      conn.channelId = ids.channelId;
      conn.avatar = ids.avatar || "";
    } catch (err) {
  ...
  const socket = new WebSocket(
    `wss://ws-us2.pusher.com/app/${KICK_PUSHER_APP_KEY}?protocol=7&client=js&version=8.4.0-rc2&flash=false`
  );
  conn.socket = socket;

  // The first Kick channel is primary (no badge, counts in stats). Additional
  // channels are mirrors: their messages carry this name as `sharedFrom` (+ the
  // streamer's avatar as `sharedAvatar`), which the feed renders as the
  // channel's profile image (or a gold "↪ name" badge fallback if no avatar).
  // The stats layer excludes mirrored messages/events.
  const isPrimary = session.kickUsers[0] === channelName;
  const sharedFrom = isPrimary ? null : channelName;
  const sharedAvatar = isPrimary || !conn.avatar ? "" : conn.avatar;

  socket.addEventListener("open", () => {
    if (session.closed) return;
    // Subscribe to the v2 chatroom channel. This carries chat messages and most
    // events (SubscriptionEvent, bans, deletes, pins).
    subscribeKickChannel(socket, `chatrooms.${conn.chatroomId}.v2`);
    // Also subscribe to the public presence channels. Kick delivers some events
    // here that the chatrooms channel does not — notably gifted subs and
    // follow/leaderboard updates. We subscribe to BOTH naming conventions
    // ("channel.{id}" dot and "channel_{id}" underscore): Kick has shipped both
    // over time, and subscribing to the extra costs nothing (the two carry
    // disjoint event sets, so there is no duplicate-delivery risk — unlike the
    // v1/v2 chatrooms split, which we avoid on purpose).
    if (conn.channelId) {
      subscribeKickChannel(socket, `channel.${conn.channelId}`);
      subscribeKickChannel(socket, `channel_${conn.channelId}`);
    }
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (parsed.chat) {
      const c = parsed.chat;
      session.pushChat("kk", c.username, c.content, c.color, c.emotes, undefined, sharedFrom, sharedAvatar);
    } else if (parsed.event) {
      const text = formatKickEvent(parsed.event);
      if (!text) return;
      parsed.event.sharedFrom = sharedFrom; // stamps the event for the feed + stats guard
      if (sharedAvatar) parsed.event.sharedAvatar = sharedAvatar;
...
function parseKickFrame(raw) {
  ...
  switch (eventName) {
    ...
    case KICK_EVENT.GIFTED_SUBS_LEGACY:
      return { event: parseKickGiftedSubs(payload) };
    case KICK_EVENT.GIFTS_LEADERBOARD:
      return null; // leaderboard updates have no per-actor feed row; consumed only for side data if needed
    ...
    case KICK_EVENT.KICKS_GIFTED:
      return { event: parseKickKicksGifted(payload) };
    default:
...
// Gifted subs. The canonical event is LuckyUsersWhoGotGiftSubscriptionsEvent
// on channel.{channel_id} (NOT on chatrooms.{id}.v2). Its payload carries the
// gifter and the recipients:
//   {
//     gifter_username: "El_Gringo_Sagan",
//     usernames: ["blunderpandaa1927", ...],   // or user_ids: [32713, ...]
//     channel: { ... }
//   }
// We also read the older GiftedSubscriptionsEvent shape (gifted_usernames[] +
// gifter_username) and a handful of camelCase variants, since Kick's payload
// has drifted over time and isn't officially documented.
function parseKickGiftedSubs(payload) {
  ...
  let count = null;
  // New payload (LuckyUsersWhoGotGiftSubscriptionsEvent): `usernames[]` array.
  // Legacy payload (GiftedSubscriptionsEvent): `gifted_usernames[]` array.
  if (Array.isArray(payload.usernames)) {
    count = payload.usernames.length;
  } else if (Array.isArray(payload.gifted_usernames)) {
    count = payload.gifted_usernames.length;
  } else if (Array.isArray(payload.user_ids)) {
    count = payload.user_ids.length;
  } else {
    const n = Number(payload.giftedCount || payload.giftee_quantity || payload.quantity || payload.amount || payload.gifted_quantity);
...
// Kicks tip (currency). Kick fires `KicksGifted` on the public
// channel.{channel_id} when a viewer sends the streamer Kicks (the Kick
// currency — the bits-equivalent). Payload shape:
//   { sender: { username, username_color }, gift: { name, amount },
//     message: "optional attached message", gift_transaction_id: ... }
// We surface it as an event row with kind "kicks", mirroring how Twitch bits
// get a distinct highlight. The optional attached message rides in `message`.
function parseKickKicksGifted(payload) {
  const sender = payload.sender || payload.user || {};
  const gift = payload.gift || {};
  const who = sender.username || sender.slug || "Someone";
...
function formatKickEvent(ev) {
  ...
  switch (ev.kind) {
    ...
    case "kicks": {
      ...
      const gname = ev.giftName || "Kicks";
      return ev.message
        ? `${name} sent${amt} ${gname}: ${ev.message}`
        : `${name} sent${amt} ${gname}!`;
    }
    default:
      return "";
...
// Extracts the real client IP for a WS upgrade request. WS connections don't
// go through Express middleware, so req.ip is NOT populated — we have to read
// X-Forwarded-For ourselves when running behind Cloudflare/nginx (the raw
// req.socket.remoteAddress would be the proxy's address for every client,
// which collapses MAX_WS_PER_IP into a global cap and DoSes the whole site).
// Takes the LEFTMOST (first) hop from XFF, matching the trust-proxy convention
// used by express-rate-limit. Falls back to the socket address otherwise.
...
wss.on("connection", (ws, req) => {
  ...
  // Read the real client IP (honors X-Forwarded-For behind a proxy). Without
  // this, behind Cloudflare/nginx every connection looks like it comes from
  // the proxy and MAX_WS_PER_IP becomes a whole-site cap.
  const clientIp = wsClientIp(req);
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "join") {
      ...
      const tw = normalizeChatHandle(msg.twitch);
      ...
      const kkChannels = [...new Set(
        String(msg.kick || "")
          .split(",")
          .map((s) => normalizeChatHandle(s))
          .filter(Boolean)
      )].slice(0, 3);
      const kk = kkChannels.length ? kkChannels.join(",") : null;
      // TikTok: strip @ or a full URL (keep the part after the last /)
      let tt = typeof msg.tiktok === "string" ? msg.tiktok.trim() : "";
      tt = tt.replace(/^@/, "");
      if (/\/|@/.test(tt)) {
        const m = tt.split(/[/@]/).filter(Boolean).pop();
        tt = m || "";
      }
      tt = normalizeChatHandle(tt);
      // YouTube: accepts a full URL or videoId — we pass it through as-is.
      // We still cap its length so a malicious client can't push a giant
      // string into the session key / extractor.
      const yt = typeof msg.youtube === "string" ? msg.youtube.trim().slice(0, 512) || null : null;
      if (!tw && !kk && !tt && !yt) {
        safeSend(ws, JSON.stringify({ type: "error", error: "Enter at least one username" }));
        return;
      }
      // Stream-ended marker: if this exact channel combination was destroyed
      // because its stream ended, do NOT silently resume. Tell the client so it
      // clears saved channels and returns to the setup screen. The marker is
      // one-shot (deleted here) so a manual re-join after re-entering channels
      // proceeds normally and we never loop the user back to setup.
      const joinKey = sessionKey(tw, kk, tt, yt);
      if (endedSessions.has(joinKey)) {
        endedSessions.delete(joinKey);
        log.info({ session: joinKey }, "session_ended: stream ended marker hit, sending to setup");
        safeSend(ws, JSON.stringify({ type: "session_ended" }));
        return;
      }
...
setInterval(() => {
  ...
  for (const session of sessions.values()) {
    ...
    if (session.clients.size === 0 && session.emptySince && now - session.emptySince >= SESSION_GRACE_MS) {
      ...
      // Mirror scheduleExpiry(): record an ended marker only if the stream was
      // confirmed ended, so a returning client is sent to the setup screen.
      if (session.streamEnded) {
        endedSessions.set(session.key, { exp: Date.now() + ENDED_SESSION_TTL_MS });
      }
      session.destroy();
  ...
  // Purge ended-session markers past their TTL (backstop — markers are also
  // one-shot deleted on first join match).
  let sweptEnded = 0;
  for (const [key, marker] of endedSessions) {
    if (marker.exp < now) {
      endedSessions.delete(key);
      sweptEnded++;
    }
  }
  if (sweptSessions || sweptTickets || sweptEnded) {
    log.info({ sweptSessions, sweptTickets, sweptEnded, activeSessions: sessions.size }, "sweep complete");
  }
