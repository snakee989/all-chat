...
function parseKickFrame(raw) {
  ...
  // TEMP DEBUG: log every Kick Pusher event name so we can see what Kick is
  // actually sending (event names shift over time). Remove once confirmed.
  // For ChatMessageEvent we also log the `type` field — Kick delivers some
  // events (gifted subs, subscriptions) as ChatMessageEvent with a non-default
  // type, so we need to see those too.
  if (!eventName.startsWith("pusher:")) {
    if (eventName === "App\\Events\\ChatMessageEvent") {
      // Only log chat messages that aren't plain chat (type !== "chat" and
      // type !== undefined). Gifted subs / sub messages have types like
      // "subscription", "gift_received", "host", etc.
      const msgType = payload?.type || payload?.message?.type;
      if (msgType && msgType !== "chat" && msgType !== "message") {
        log.info({ event: eventName, channel: outer.channel, msgType, payload }, "kick special-type chat event");
      }
    } else {
      const isSubLike = /sub|gift|leaderboard|lucky/i.test(eventName);
      log.info(
        { event: eventName, channel: outer.channel, payload: isSubLike ? payload : undefined },
        "kick pusher event (non-chat)"
      );
    }
  }
