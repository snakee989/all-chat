...
class Session {
  ...
  // sharedFrom: optional string — Twitch Shared Chat origin channel login, when this
  //   message was mirrored from a linked room (rendered as a "↪ name" badge)
  ...
  pushEvent(platform, kind, who, text, info) {
    ...
    if (info && info.sharedFrom) {
      msg.sharedFrom = info.sharedFrom; // Twitch Shared Chat origin channel
    }
    this.backlog.push(msg);
    while (this.backlog.length > BACKLOG_LIMIT) this.backlog.shift();
    this.broadcast(msg);
