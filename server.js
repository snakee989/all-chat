...
// session (cancelled if a client returns). Kept long (6h) deliberately: the
// aggregate stats (subs/gifts/bits) are tied to the session's lifetime, and a
// stream runs for hours — a viewer who closes the browser for a meeting or a
// nap should not lose the stream's running totals. Platform connections stay
// open (and keep capturing events) throughout this window.
const SESSION_GRACE_MS = 6 * 60 * 60 * 1000; // 6 hours
...
class Session {
  constructor(twitchUser, kickUser, tiktokUser, youtubeInput) {
    ...
    this.platforms = {
      twitch: { socket: null, status: "connecting", reconnecting: false, timer: null, error: null, viewers: null, viewerTimer: null },
      kick: {
        status: "connecting", // aggregate status derived from per-channel conns
        error: null,
        conns: new Map(), // channelName -> { socket, chatroomId, pingTimer, reconnectTimer, reconnecting, status, error, viewers, viewerTimer }
      },
...
async function fetchKickViewerCount(username) {
  try {
    ...
    if (!r.ok) return null;
    const data = await r.json();
    const live = Number(data?.livestream?.viewer_count);
    if (Number.isFinite(live) && live >= 0) return live;
    const top = Number(data?.viewerCount);
...
function connectTwitch(session) {
  ...
  socket.addEventListener("message", async (event) => {
    ...
    for (const line of lines) {
      ...
      if (parsed.message) {
        // Resolve a Shared Chat origin channel id to a login before emitting,
        // so mirrored messages are labelled with "↪ channelName". Awaiting in
        // order preserves message ordering; the lookup is cached and near-free
        // after the first occurrence.
        let sharedFrom;
        if (parsed.message.sharedRoomId) {
          const login = await resolveChannelLogin(parsed.message.sharedRoomId);
          sharedFrom = login || "shared"; // generic fallback if Helix is unconfigured/unavailable
        }
        session.pushChat(
          "tw",
      ...
      if (parsed.event) {
        ...
        if (!text) continue;
        if (parsed.event.sharedRoomId) {
          const login = await resolveChannelLogin(parsed.event.sharedRoomId);
          parsed.event.sharedFrom = login || "shared";
        }
        session.pushEvent("tw", parsed.event.kind, parsed.event.who, text, parsed.event);
      }
    }
  });
...
async function fetchTwitchViewerCount(login) {
  if (!TWITCH_CLIENT_ID || !login) return null;
  try {
    const token = await getTwitchAppAccessToken();
    if (!token) return null;
    const r = await fetchWithTimeout(
    ...
    const count = stream ? Number(stream.viewer_count) : null;
...
async function pollTwitchViewers(session) {
  ...
  const count = await fetchTwitchViewerCount(session.twitchUser);
  if (session.closed) return;
...
function setKickConnStatus(session, channelName, status, error) {
  ...
  if (!conn) return;
  conn.status = status;
  if (error !== undefined) conn.error = error || null;
...
async function connectKickChannel(session, channelName) {
  ...
  socket.addEventListener("open", () => {
    ...
    subscribeKickChannel(socket, `chatrooms.${conn.chatroomId}`);
...
async function pollKickViewers(session, channelName) {
  ...
  const count = await fetchKickViewerCount(channelName);
  if (session.closed) return;
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", (raw) => {
    ...
    // no per-client ack is sent. Ignored before a session is joined.
    if (msg.type === "reset_stats") {
      if (session) session.resetStats();
      return;
    }
  });
