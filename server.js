...
app.get("/auth/twitch", oauthLimiter, (req, res) => {
  ...
  const params = new URLSearchParams({
    ...
    response_type: "code",
    // offline_access is required for Twitch to return a refresh_token so we can
    // silently renew the access token before it expires (it otherwise dies in
    // ~1h and forces the viewer to re-authorize mid-stream).
    scope: "chat:read chat:edit user:read:chat openid offline_access",
    state,
    force_verify: "true",
  });
