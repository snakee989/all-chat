...
app.get("/auth/twitch/callback", oauthLimiter, async (req, res) => {
  ...
  // Persist the refresh_token (if Twitch returned one — requires the
  // offline_access scope) in a long-lived HttpOnly cookie on the server. We
  // never expose it to JS; the client asks POST /api/twitch-refresh to renew.
...
async function connectKickChannel(session, channelName) {
  ...
  socket.addEventListener("open", () => {
    ...
    // Also subscribe to the public channel.{channel_id}. Kick delivers some
    // events here that the chatrooms channel does not — notably gifted subs and
    // follow/leaderboard updates. The two channels carry disjoint event sets, so
    // there is no duplicate-delivery risk (unlike the v1/v2 chatrooms split).
    if (conn.channelId) {
...
function parseKickFrame(raw) {
  ...
  // TEMP DIAGNOSTIC (max): log EVERY non-chat Kick Pusher event with its full
  // payload, plus any ChatMessageEvent that carries a non-default `type`. Kick's
  // unofficial API drifts and some pusher-js forks normalize event names, so we
  // dump everything to find the true event name that carries gifted subs.
  if (!eventName.startsWith("pusher:")) {
    if (eventName === "App\\Events\\ChatMessageEvent") {
      // Still skip plain chat to avoid log spam, but capture any with a type.
      const msgType = payload?.type || payload?.message?.type;
      if (msgType && msgType !== "chat" && msgType !== "message") {
        log.info({ event: eventName, channel: outer.channel, msgType, payload }, "kick event (typed chat)");
      }
    } else {
      log.info({ event: eventName, channel: outer.channel, payload }, "kick event (all)");
    }
  }
