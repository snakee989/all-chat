...
//   4) نتحقق من الـ token ونجلب اسم المستخدم، ثم نخزّنه في كوكي مؤقت
//      (لمرة واحدة، httpOnly) ونُعيد التوجيه للصفحة الرئيسية. العميل يلتقط
//      الـ token عبر /api/twitch-token ثم يُحفظه في localStorage.
// ----------------------------------------------------------------------
...
// رد تويتش: نتبادل code بـ token، ثم نخزّنه في كوكي مؤقت ونُعيد التوجيه للصفحة.
app.get("/auth/twitch/callback", async (req, res) => {
  ...
  // نخزّن النتيجة في خريطة في الذاكرة تحت مفتاح عشوائي، ونضعه في كوكي مؤقت
  // (30 ثانية) httpOnly. هذا أكثر موثوقية من تمرير الـ token في الـ URL fragment
  // الذي قد يُمْسَح عبر بروكسي/HTTPS ولا يصل للمتصفح.
...
// عند فتح الصفحة بعد العودة من تويتش: العميل يسأل هذا المسار فيقرأ الكوكي
// ويستلم الـ token (ثم يُلغى التذكرة فوراً = استخدام لمرة واحدة).
app.get("/api/twitch-token", (req, res) => {
  ...
  oauthTickets.delete(ticket); // استخدام لمرة واحدة
...
// تخزين تذاكر OAuth مؤقتاً (في الذاكرة) — مفتاح عشوائي -> {token, login, exp}
const oauthTickets = new Map();
function cryptoRandom() {
  return require("crypto").randomBytes(18).toString("base64url");
}
// يبني كوكي بنفس سمة SameSite المختارة (lax افتراضياً؛ للنشر عبر نطاق فرعي
// قد تحتاج none، لكن none يتطلّب Secure).
function authCookieAttributes(maxAgeSec) {
  const parts = ["Path=/", `Max-Age=${maxAgeSec}`, "SameSite=Lax", "HttpOnly"];
  // خلف HTTPS نضيف Secure ليُرسل الكوكي فقط عبر اتصال مشفّر
...
// عند فشل OAuth نُوجّه للصفحة مع علامة خطأ في كوكي مؤقت يقرأه العميل.
