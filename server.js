...
async function connectKickChannel(session, channelName) {
  ...
  // Resolve chatroom_id + channel_id once and reuse them across reconnects.
  if (!conn.chatroomId) {
    try {
...
function parseKickFrame(raw) {
  ...
  // TEMP DIAGNOSTIC (max): also surface Pusher's own subscribe ack/error frames.
  // These were previously silently dropped by the `!eventName.startsWith("pusher:")`
  // guard below, which meant a failed subscription to channel.{channelId} (the
  // channel gifted-sub events depend on) would fail *silently* — chat/bans still
  // work fine because those live on chatrooms.{id}.v2, a separate subscription.
  if (eventName === "pusher_internal:subscription_succeeded" || eventName === "pusher:subscription_error") {
    log.info({ event: eventName, channel: outer.channel, payload }, "kick pusher subscribe ack");
  }
