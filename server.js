...
app.post("/api/twitch-refresh", apiLimiter, async (req, res) => {
  ...
  try {
    ...
    return res.json({ ok: false, reauth: false, retry: true });
  }
  if (!r.ok) {
  ...
  if (typeof j.refresh_token === "string" && j.refresh_token) {
    setAuthCookie(res, "tw_refresh", j.refresh_token, 365 * 24 * 60 * 60);
  }
  ...
  res.json({ ok: true, access_token: accessToken, login: "", expires_in: expiresIn });
});
