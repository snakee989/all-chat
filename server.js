...
class Session {
  ...
  addClient(ws) {
    this.clients.add(ws);
    // أرسل له الرسائل السابقة + الحالة الحالية فوراً
    ws.send(
      JSON.stringify({
        type: "snapshot",
  ...
  pushChat(platform, username, content, color) {
    const msg = {
      type: "chat",
      platform,
      username,
...
function connectTwitch(session) {
  ...
  socket.addEventListener("message", (event) => {
    ...
    for (const line of lines) {
      ...
      if (parsed.message) {
        session.pushChat("tw", parsed.message.username, parsed.message.content, parsed.message.color);
      }
    }
  });
...
function parseTwitchLine(line) {
  ...
  return { message: { username, content, color } };
}
...
async function connectKick(session) {
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (parsed) session.pushChat("kk", parsed.username, parsed.content, parsed.color);
  });
...
function parseKickFrame(raw) {
  ...
  return { id, username, content, color };
}
